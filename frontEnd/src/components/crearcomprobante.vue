              ...
              <table class ="table table-striped w-auto">
                  ...
                  <tbody>
                        <tr v-for="det in variablesEntorno.list" :key="det.businessName">
                            <td>{{ det.customer.businessName }}</td>
                            <td>{{ det.customerId }}</td> 
                            <td>{{ det.customer.cuit }}</td>
...
export default {
  ...
  methods: {
                ...
                .then(response => {
                    if (response.data.length <1){
                      ...
                      alert("No se encontraron resultados");
                        ...
                        variablesEntorno.cuitView = variablesEntorno.list[0].customer.cuit;
                        variablesEntorno.condImpositiva=variablesEntorno.list[0].taxConditionId; 
                        variablesEntorno.razonSocialView=variablesEntorno.list[0].customer.businessName;
                        variablesEntorno.idcliView=variablesEntorno.list[0].customerId;
                        variablesEntorno.condImpositivaView = DevolverTexto(variablesEntorno.list[0].customer.taxConditionId)[0];
                        if (variablesEntorno.list[0].customer.taxConditionid == 1){
      ...
      SeleccionarCliente(det){
            variablesEntorno.showModal=false;
            variablesEntorno.cuitView = det.customer.cuit;
            variablesEntorno.condImpositiva=det.customer.taxConditionId;
             variablesEntorno.condImpositivaView = DevolverTexto(det.customer.taxConditionId)[0];
            variablesEntorno.razonSocialView=det.customer.businessName;
            variablesEntorno.idcliView=det.customerId;
            if (det.customer.taxConditionId == 1){
                variablesEntorno.tipoComprobanteView = 'Factura A'
                variablesEntorno.facturaA = true;
                } else {
                    variablesEntorno.tipoComprobanteView = 'Factura B'
